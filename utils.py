    ...
    with open(infile, 'r') as csvfile:
        ...
        for row in reader:
            ...
            if doc_level:
                ...
                indset = indices[note_inds[-2]:note_inds[-1]]
...
def prepare_code_title(dicts, args, max_length):
    w2ind, c2ind = dicts['w2ind'], dicts['c2ind'][4]
    dict_code_title = {}

    # get diagnoses code titles
    c2title = {}
    ...
    # handle code without titles. This happens because there are:
    # wrong code like 36.01 that does not belong to ICD9
    # low hierarchy code like 37.1 that belongs to ICD9 but is not included in D_ICD_PROCEDURES.csv
    for code, _ in c2ind.items():
        if code not in c2title.keys():
            c2title[code] = ''

    for code, title in c2title.items():
        tokens_ = title.split()
        tokens = []
        tokens_id = []
        ...
        padded_tokens_id = np.zeros(max_length, dtype=type)
    ...
    for code_id, code in ind2c.items():
        code_title.append(dict_code_title[code])
...
def prepare_instance(dicts, filename, args, max_length):
    ...
    instances = []
    num_labels = [len(ind2c[i]) for i in range(5)]
